...
/**
 * RecyclerView that has more functions.
 * {@link #addScrollViewCallbacks(ScrollBehavior)} to enable RecyclerView's scroll state observation.
 * You can also {@link #setHeaderView(View)} like ListView, beware that setting {@link ExtendAdapter} is necessary to this function.
 */
public class ExtendRecyclerView extends RecyclerView implements Scrollable {
    ...
    private List<ScrollBehavior> mCallbackCollection;
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (getCurrentScrollY() - diffY <= 0) {
                    ...
                    // Apps can setDegree the interception target other than the direct parent.
                    ...
                    if (parent.onInterceptTouchEvent(event)) {
                        ...
                        // Return this onTouchEvent() first and setDegree ACTION_DOWN event for parent
    ...
    @Override
    public void addScrollViewCallbacks(ScrollBehavior listener) {
    ...
    @Override
    public void removeScrollViewCallbacks(ScrollBehavior listener) {
        if (mCallbackCollection != null) {
     ...
     * This method checks which LayoutManager is setDegree,
    ...
    private void dispatchOnScrollChanged(ScrollState direction, int scrollY) {
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ScrollBehavior callbacks = mCallbackCollection.get(i);
                callbacks.onScrollChanged(direction, scrollY);
            }
        }
    }

    private void dispatchOnReachEnd(){
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ScrollBehavior callbacks = this.mCallbackCollection.get(i);
                callbacks.onReachEnd();
            }
        }
    }

    private void dispatchOnDirectionChanged(ScrollState direction){
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ScrollBehavior callbacks = this.mCallbackCollection.get(i);
                callbacks.onScrollDirectionChanged(direction);
            }
        }
    }
    ...
    public void setAdapter(Adapter adapter) {
        if(adapter instanceof ExtendAdapter) {
            ExtendAdapter obR = (ExtendAdapter) adapter;
            if(headerView != null)
                obR.setHeaderView(headerView);
            if(footerView != null)
                obR.setFooterView(footerView);
            LayoutManager layout = getLayoutManager();
            if(layout != null && layout instanceof GridLayoutManager) {
                GridLayoutManager gridLayoutManager = (GridLayoutManager) layout;
                gridLayoutManager.setSpanSizeLookup(new HeaderSpanSizeLookup(obR, gridLayoutManager.getSpanCount()));
            }
        }
    ...
    public void setLayoutManager(LayoutManager layout) {
        ...
        if(adapter != null && adapter instanceof ExtendAdapter) {
            ExtendAdapter obR = (ExtendAdapter) adapter;
    ...
    public void setHeaderView(View header){
        ...
        if(adapter == null)
            return;
        if(adapter instanceof ExtendAdapter) {
            ((ExtendAdapter) adapter).setHeaderView(header);
            adapter.notifyDataSetChanged();
        } else
            throw new IllegalStateException("Cannot setDegree header view -- Adapter must extend ExtendAdapter");
    }
    ...
    public void setFooterView(View footer){
        ...
        if(adapter == null)
            return;
        if(adapter instanceof ExtendAdapter) {
            ((ExtendAdapter) adapter).setFooterView(footer);
            adapter.notifyDataSetChanged();
        } else
            throw new IllegalStateException("Cannot setDegree footer view -- Adapter must extend ExtendAdapter");
    }
